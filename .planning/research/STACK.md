...
**Project:** Database Deployment Automation — PostgreSQL migration safety pipeline
**Researched:** 2026-09-06
**Confidence:** HIGH on core versions and CLI surfaces (verified via GitHub API and official docs); MEDIUM on ecosystem-tool behavior (verified via web search cross-checks); explicit LOW flags called out inline.
...
**Important version note — do NOT use Drizzle v1.0.0-beta/rc:** Drizzle ORM and Drizzle Kit are mid-rewrite toward v1.0. As of this research date the newest tag is `v1.0.0-rc.4` (2026-06-27), but it is explicitly documented by the Drizzle team as **not recommended for production**, with a stated downgrade path back to 0.44.x/beta if it breaks. The v1 line also removes `_journal.json` entirely and restructures the migrations folder (see "What Drizzle does NOT give you" below) — a breaking change to the exact mechanism this project's audit trail depends on. **Recommendation: pin to the 0.x stable line (`drizzle-orm@0.45.2`, `drizzle-kit@0.31.10`) for this project's v1 and revisit the v1.0 GA only after it ships stable and this pipeline is proven.** Confidence: HIGH (GitHub Releases API, direct).
...
pnpm add -D squawk-cli

# Testing
pnpm add -D vitest @testcontainers/postgresql

# Pipeline plumbing
pnpm add zod execa pino
...
**Core safety analyzer — reuse vs. build (the biggest decision in this project):**
...
**If the pipeline later needs actual lock-duration evidence (not just static classification):**
...
**If/when the project needs true zero-downtime multi-release migrations (expand/contract with dual schema access):**
...
**For getting `psql`/`pg_dump`/`pg_restore` on Windows without a local Postgres install:**
...
**For the "empty DB → full migration history" and "existing DB → new migration" tests:**
...
---
*Stack research for: PostgreSQL migration safety pipeline (Drizzle ORM, Node/TypeScript, AI-agent-driven development, self-hosted Coolify/Hetzner)*
*Researched: 2026-09-06*
