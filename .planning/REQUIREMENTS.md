...
- [x] **RUN-03**: Every migration execution is wrapped with `lock_timeout` and `statement_timeout`
...
- [x] **CI-03**: The gate is enforced by a GitHub repository ruleset with an empty bypass list, so it cannot be waved through by the repository owner
- [x] **CI-04**: The safety classification and its reasoning are surfaced on the pull request itself
- [x] **CI-05**: Hand-edited migration files, and edits to already-applied migrations, are detected mechanically
- [x] **CI-06**: Migrations run as their own pipeline step and never at application startup
...
| CI-01 … CI-06 | Phase 5 | All 6 complete 2026-09-09. CI-01: `.github/workflows/pr-gate.yml`'s `analyze`, `test`, `test-history`, and `migrate` jobs are all required contexts on `main-protection` and were all observed reporting on real pull requests (`#3`, `#4`, `#7`). CI-02: `docs/40-ci-gate-merge-attempt.md` Observation 1 — PR #4's `analyze` job reported BLOCKED on a real `DROP TABLE` migration and the build failed. CI-03: `docs/decisions.md` D26 (ruleset live, `bypass_actors` read back present and empty, direct push refused) plus `docs/40-ci-gate-merge-attempt.md` Observation 1 (the owner personally attempted the merge with their own admin account and could not complete it; no bypass affordance was offered). CI-04: the sticky comment quoted verbatim in `docs/40-ci-gate-merge-attempt.md` Observation 1, naming every rule id and rationale directly on the pull request. CI-05: `docs/40-ci-gate-merge-attempt.md` Observations 2 and 3 — both tamper checks (append-only, schema-drift) were falsified against real pull requests (`#5`, `#6`), not only fixtures. CI-06: `docs/decisions.md` D29 (the `migrate` job is a structurally separate pipeline step, live on PR `#3`) plus `.github/workflows/pr-gate.yml`'s own shape (the `migrate` job's only step is `pnpm db:migrate`; no build/seed/test/boot) plus `tests/guardrails.test.ts`'s D-15 guardrail (no application source or startup-hook file references the migration entry point). No requirement was ticked on implementation alone — each cites a specific observed artifact. |
