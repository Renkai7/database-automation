...
describe("dev database schema scope (ENV-02, D-09/D-10/D-11)", () => {
  it("public.recipes has exactly its ten current columns, no more and no fewer", async () => {
    const client = new Client({ connectionString: getDevDatabaseUrl() });
    await client.connect();
    try {
      await assertDevelopmentDatabase(client);
      ...
      // 04-06/D-30: `notes` is now a deliberately spent, recorded reserved-churn column (a
      // nullable ADD COLUMN generated by drizzle-kit and applied through the runner, landing
      // SAFE) -- the Phase 1 nine-column baseline is updated here to reflect that decision, not
      // silently relaxed. The `tags` table alternative and the expand-and-contract row remain
      // unspent.
      ...
      expect(
        actualColumns,
        "D-09/D-10/D-11 (04-06/D-30): public.recipes's column set must match the recorded " +
          "baseline exactly -- `notes` was deliberately spent this phase; any further addition, " +
          "removal, or rename here must be its own deliberate, recorded decision, not a silent " +
          "drift",
