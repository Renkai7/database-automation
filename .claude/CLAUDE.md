...
| `libpg-query` | **18.1.4** (upgraded from 17.7.4 during Phase 3 implementation — commit `9f9ff0f`; see PostgreSQL row below) | Real PostgreSQL parser (WASM) → AST for the safety analyzer | The literal Postgres C grammar compiled to WebAssembly. **This is the core dependency for the whole project's safety-analyzer decision** — see the dedicated section below. |
...
| Technology | Version | Purpose | Why |
|------------|---------|---------|-----|
| PostgreSQL | Dev/local runs PostgreSQL 17 (D9, `docs/decisions.md`). **The production major version remains UNKNOWN as of Phase 3** — this gap was flagged here at research time and was never closed; recorded again, explicitly, in Phase 3's `docs/decisions.md` D16 rather than left to go stale silently. | Target database | Project constraint. Pin the exact major version across dev/staging/prod; it determines which `libpg-query`/`pgsql-parser` PG-version dist-tag to install and which `postgres:*-alpine` client image to use for backup/restore tooling. Phase 3's analyzer parses with `libpg-query`'s pg18 grammar (see the row above) while the pinned dev/CI server stays PostgreSQL 17 (D9) — a newer parser grammar is a near-superset of an older server's, so the failure mode of that mismatch is accepting syntax an older server would reject, never missing a real hazard like `DROP TABLE`. That makes parsing-newer-than-target the safe direction for a safety analyzer specifically, not a discrepancy to silently paper over. |
...
| Technology | Version | Purpose | Why |
|------------|---------|---------|-----|
| `libpg-query` | 18.1.4 (upgraded from 17.7.4 during Phase 3; the pg13–17 dist-tag lines are parse-only, and `parsePlPgSQL` — needed for D-05's recursion into DO-block/function bodies — ships only on the pg18 line) | Parse migration SQL into a real Postgres AST | See "SQL parsing" section. Build the safety classifier on top of this — it is the single most important dependency in the stack. |
...
| `libpg-query` | Postgres major version via npm dist-tag | The package defaults to the PG18 API; install `libpg-query@pg17` (or the tag matching your actual server) explicitly if the project's Postgres major version differs, to keep parser grammar and target server aligned. **Confirm the actual Postgres version Coolify/production will run before pinning this** — not yet established in this research pass. **Phase 3 shipped on the default (pg18) dist-tag deliberately**: dev/CI is pinned to PostgreSQL 17 (D9), and a newer parser grammar is a near-superset of an older server's, so the failure direction of that mismatch is accepting syntax an older server would reject — never missing a real hazard. Production's major version is still UNKNOWN (see the gap flag below and `docs/decisions.md` D16); this reasoning is why that gap did not block Phase 3, not a reason to stop tracking it. |
